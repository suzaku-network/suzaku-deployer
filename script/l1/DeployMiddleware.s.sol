...
contract DeployMiddlewareL1 is Script {
    ...
    function run(string memory inputJsonPath) external {
        ...
        middlewareConfig.middlewareOwnerAddress = jsonData.readAddress(
            ".roles.middlewareOwner_middleware"
        );
        middlewareConfig.validatorManager = jsonData.readAddress(
            ".deployed.validatorManager"
        );
        middlewareConfig.operatorRegistry = jsonData.readAddress(
            ".deployed.operatorRegistry"
        );
        middlewareConfig.vaultFactory = jsonData.readAddress(
            ".deployed.vaultFactory"
        );
        middlewareConfig.operatorL1OptIn = jsonData.readAddress(
            ".deployed.operatorL1OptIn"
        );
        middlewareConfig.primaryCollateral = jsonData.readAddress(
            ".deployed.primaryCollateral"
        );
        middlewareConfig.primaryCollateralMaxStake = jsonData.readUint(
            ".middleware.primaryCollateralMaxStake"
        );
        middlewareConfig.primaryCollateralMinStake = jsonData.readUint(
            ".middleware.primaryCollateralMinStake"
        );
        middlewareConfig.primaryCollateralWeightScaleFactor = jsonData.readUint(
            ".middleware.primaryCollateralWeightScaleFactor"
        );
        middlewareConfig.epochDuration = uint48(
            jsonData.readUint(".middleware.epochDuration")
        );
        middlewareConfig.slashingWindow = uint48(
            jsonData.readUint(".middleware.slashingWindow")
        );
        middlewareConfig.stakeUpdateWindow = uint48(
            jsonData.readUint(".middleware.stakeUpdateWindow")
        );
        middlewareConfig.vaultRemovalEpochDelay = uint48(
            jsonData.readUint(".middleware.vaultRemovalEpochDelay")
        );

        // Deploy
        console2.log("Deploying middleware...");
        DeployTestAvalancheL1Middleware deployMiddleware = new DeployTestAvalancheL1Middleware();
        (address middleware, address vaultManager) = deployMiddleware
